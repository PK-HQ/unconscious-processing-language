%
%PAL_AMUD_up_analyzeUD_up  Determines threshold estimates based on up/down
%   adaptive method
%   
%   syntax: Mean = PAL_AMUD_up_analyzeUD_up(UD_up, {optional arguments})
%
%   After having completed an up/down staircase PAL_AMUD_up_analyzeUD_up may be
...
%   By default,
%
%   Mean = PAL_AMUD_up_analyzeUD_up(UD_up) will calculate the mean of all but the
%       first two reversals.
%
...
%
%   Mean = PAL_AMUD_up_analyzeUD_up(UD_up, 'reversals',10) will calculate the mean 
%       of the last 10 reversals.
%
%   and,
%
%   Mean = PAL_AMUD_up_analyzeUD_up(UD_up, 'trials',25) will calculate the mean 
%       of the last 25 trials.
%
%Introduced: Palamedes version 1.0.0 (NP)
%Modified: Palamedes version 1.6.3 (see History.m)

function Mean = PAL_AMUD_analyzeUD_up(UD_up, varargin)

HighReversal = max(UD_up.reversal);
NumTrials = length(UD_up.response);
...
if strncmpi(criterion,'reversals',4)
    Mean = sum(UD_up.xStaircase(UD_up.reversal >= LowReversal))/(HighReversal-LowReversal+1);
else
    Mean = sum(UD_up.xStaircase(LowTrial:NumTrials))/(NumTrials-LowTrial+1);
end