...
location              =input('Location Top1 Bottom2: ');
...
up_or_down=[];
for i=1:5,
    randgen=repmat([1 2]',6,1);
    randgen = randgen(randperm(length(randgen)),:);
    up_or_down=[up_or_down; randgen];
end
...
ntrials  = 60;
norep=4; x=(1:15)'; % example
r=repmat(x,1,norep)';
sentenceID=r(:);
persentencecond=repmat([1,1,location_cued;1,2,location_cued;2,1,location_cued;2,2,location_cued;1,1,location_cued;1,2,location_cued;2,1,location_cued;2,2,location_cued;1,1,location_uncued;1,2,location_uncued;2,1,location_uncued;2,2,location_uncued],5,1); %2:1 ratio
CondList=[sentenceID, persentencecond];
CondList = CondList(randperm(length(CondList)),:);
CondList = [up_or_down CondList]; % combining into final Colndition List
CondList=[(1:ntrials)',CondList(:,1:5)];
a=CondList(:,3);
...
%% Script start
for i=1:ntrials, %total trials for both staircases
    pause(0.3)
    %% Ascending staircase
    if CondList(i,2)==1,
        while ~UD_up.stop,
            try
                text3=Sentence_calibration(CondList(i,3),CondList(i,4));
