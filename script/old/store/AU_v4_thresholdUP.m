...
fName_r=['FC_cfs_Sub' num2str(subjNo) '_' current_date '_' current_time '_(Cal_U).csv'];
% ***** OPEN SCREEN *****
...
%UD_up Threshold Staircase
UD_up =PAL_AMUD_up_setupUD_up('startvalue',0.1,'up',1,'down',3,'stepsizeup',0.01,'stepsizedown',0.01,'stopcriterion','trials','stopRule',40);
            ...
            while 1, %-37 & +25
                ...
                if keydown==0
                    if (GetSecs-(timezero)>Wrdtime+SOA),
                        ...
                        response = 0;
                        UD_up = PAL_AMUD_up_updateUD_up(UD_up, response);
                        disp('up here')
                        keydown=3;
                    end
...
Mean = PAL_AMUD_up_analyzeUD_up(UD_up, 'reversals', 4);
